    ...
    <main className="px-4 sm:px-6 pt-12 sm:pt-16 max-w-md mx-auto min-h-screen bg-black text-white pb-32">
      ...
      <section className="flex flex-col items-center mb-12 sm:mb-16 relative py-4">
        <Dialog>
          <DialogTrigger asChild>
            <div className={cn(
              "cursor-pointer active:scale-95 transition-all duration-500 rounded-full p-1",
              isCritical ? "animate-pulse shadow-[0_0_30px_rgba(255,0,60,0.6)] border-2 border-destructive" : ""
            )}>
              <CircularProgress 
              ...
              >
                <div className="flex flex-col items-center">
                  <span className={cn(
                    "text-[8px] sm:text-[10px] uppercase font-black tracking-[0.5em] mb-1 text-center",
                    isCritical ? "text-destructive animate-bounce" : "text-white/40"
                  )}>
                    {isCritical ? "ALERTE SYSTÈME" : "Flux Énergie"}
                  </span>
                  <span className={cn(
                    "text-4xl sm:text-6xl font-black tracking-tighter neon-text-red",
                    isCritical ? "text-destructive" : ""
                  )}>
                    {dailyLog.calories}
                  </span>
                  <div className={cn(
                    "w-16 sm:w-20 h-[1px] my-4 sm:my-5 shadow-[0_0_20px_rgba(255,0,60,0.8)]",
                    isCritical ? "bg-destructive animate-pulse" : "bg-destructive/50"
                  )} />
                  <span className="text-[8px] sm:text-[9px] text-primary uppercase font-black tracking-[0.3em] sm:tracking-[0.4em] neon-text-yellow">
                    Objectif {goals.calories}
                  </span>
                </div>
              </CircularProgress>
            </div>
          </DialogTrigger>
          <DialogContent className="bg-black/95 border-destructive neon-glow-red rounded-none p-8 max-w-[90vw] border-2">
            <DialogHeader>
              <DialogTitle className="text-destructive font-black uppercase tracking-[0.4em] text-sm mb-4">Diagnostic Énergie</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 font-mono">
              <p className="text-xs font-black uppercase leading-relaxed text-white">
                STATUT : {isCritical ? "CRITIQUE - DÉFICIT EXTRÊME" : "NOMINAL"}
              </p>
              <p className="text-[10px] font-medium uppercase leading-relaxed text-white/70">
                DÉPENSE QUOTIDIENNE TOTALE CALCULÉE POUR VOTRE PROFIL BIOMÉTRIQUE. UN APPORT NUL APRÈS 19H ENTRAÎNE UN CATABOLISME MUSCULAIRE.
              </p>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <section className="grid grid-cols-3 gap-2 sm:gap-6 mb-12">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <div className="flex flex-col items-center text-center space-y-3 cursor-pointer active:scale-95 transition-transform">
              <div className="w-10 h-10 sm:w-12 sm:h-12 border border-primary/40 flex items-center justify-center bg-black shadow-[0_0_15px_rgba(253,224,71,0.2)]">
          ...
          <DialogContent className="bg-black/95 border-primary neon-glow-yellow rounded-none p-8 max-w-[90vw] border-2 font-mono">
            <DialogHeader>
              <DialogTitle className="text-primary font-black uppercase tracking-[0.4em] text-sm mb-4">Synthèse Protéique</DialogTitle>
            </DialogHeader>
            <p className="text-xs font-black uppercase leading-relaxed text-white">CIBLE : {goals.protein}G POUR LE MAINTIEN DE LA MASSE SÈCHE.</p>
          </DialogContent>
        </Dialog>
        ...
        <Dialog>
          <DialogTrigger asChild>
            <div className="flex flex-col items-center text-center space-y-3 border-x border-white/5 px-1 cursor-pointer active:scale-95 transition-transform">
              <div className="w-10 h-10 sm:w-12 sm:h-12 border border-accent/40 flex items-center justify-center bg-black shadow-[0_0_15px_rgba(0,242,255,0.2)]">
          ...
          <DialogContent className="bg-black/95 border-accent neon-glow-blue rounded-none p-8 max-w-[90vw] border-2 font-mono">
            <DialogHeader>
              <DialogTitle className="text-accent font-black uppercase tracking-[0.4em] text-sm mb-4">Flux Glucides</DialogTitle>
            </DialogHeader>
            <p className="text-xs font-black uppercase leading-relaxed text-white">CIBLE : {goals.carbs}G POUR LE CARBURANT NEURAL.</p>
          </DialogContent>
        </Dialog>
        ...
        <Dialog>
          <DialogTrigger asChild>
            <div className="flex flex-col items-center text-center space-y-3 cursor-pointer active:scale-95 transition-transform">
              <div className="w-10 h-10 sm:w-12 sm:h-12 border border-[#a855f7]/40 flex items-center justify-center bg-black shadow-[0_0_15px_rgba(168,85,247,0.2)]">
          ...
          <DialogContent className="bg-black/95 border-[#a855f7] neon-glow-violet rounded-none p-8 max-w-[90vw] border-2 font-mono">
            <DialogHeader>
              <DialogTitle className="text-[#a855f7] font-black uppercase tracking-[0.4em] text-sm mb-4">Système Lipidique</DialogTitle>
            </DialogHeader>
            <p className="text-xs font-black uppercase leading-relaxed text-white">CIBLE : {goals.fat}G POUR L'INTÉGRITÉ CELLULAIRE.</p>
          </DialogContent>
        </Dialog>
      </section>
